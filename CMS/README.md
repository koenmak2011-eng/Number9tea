...
| `large_price`   | Optional large-size price; blank for one-size items |
| `iced_price`    | Optional iced price when it differs from hot        |
...
Use the optional `hot_surcharge` category field when every hot drink in that group has the same supplement, such as `0.20` for bubble teas.
